...
def main():
    ...
    # Log prompt information to wandb
    wandb.config.update({
        "prompt_persona": baseline_prompt.persona,
        "prompt_task": baseline_prompt.task_instructions,
        "prompt_format": baseline_prompt.output_format
    })

    # First evaluate with temperature=0 (greedy decoding)
    ...
    # Evaluate across different temperature settings
    temp_accuracies = []
    temp_refusals = []
    temperatures = [i/10 for i in range(1, 11)]
    
    for temp in temperatures:
        accuracy, refusal_rate = evaluate_prefix(prefix, model, tokenizer, device, temperature=temp)
        temp_accuracies.append(accuracy)
        temp_refusals.append(refusal_rate)
        print(f"\nResults (Temperature={temp:.1f}):")
        print(f"Accuracy: {accuracy:.4f}")
        print(f"Refusal Rate: {refusal_rate:.4f}")
    
    # Create summary metrics
    best_temp_idx = np.argmax(temp_accuracies)
    best_temp = temperatures[best_temp_idx]
    best_accuracy = temp_accuracies[best_temp_idx]
    
    # Log summary metrics
    wandb.run.summary["best_temperature"] = best_temp
    wandb.run.summary["best_accuracy"] = best_accuracy
    wandb.run.summary["temperature_zero_accuracy"] = accuracy
