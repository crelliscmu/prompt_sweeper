...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        elif generation_mode in (GenerationMode.SAMPLE_INFERENCE, GenerationMode.GREEDY_SEARCH_INFERENCE):
    ...
    def prepare_embeds_for_generation(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        past_key_values: Optional[Cache] = None,
        attention_mask: Optional[torch.LongTensor] = None,
    ...
    def _sample_inference(
        ...
        **model_kwargs,
        ...
        raw_logits = torch.empty((batch_size, 0, self.config.vocab_size), device=device)
        if 'prefix' in model_kwargs:
            prefix = model_kwargs['prefix'].squeeze(0)
            input_ids_embedding = self.model.embed_tokens(input_ids)
            embedding = torch.cat((prefix.expand(batch_size, -1, -1), input_ids_embedding), dim=1)
            model_kwargs['prefix'] = embedding
            input_ids = None
        else:
            embedding = None
            model_kwargs['prefix'] = None
        

        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=device):
            # prepare model inputs
            model_inputs = self.prepare_embeds_for_generation(input_ids = output, inputs_embeds = embedding, **model_kwargs)
            ...
            unfinished_sequences = unfinished_sequences & ~stopping_criteria(output, scores)
            this_peer_finished = unfinished_sequences.max() == 0
    ...
    def update_model_inputs(self, model_inputs, next_token_embeds, device):
        model_inputs['attention_mask'] = torch.cat((model_inputs['attention_mask'], torch.ones((model_inputs['inputs_embeds'].size(0), 1), device=device)), dim=1)
        #model_inputs['position_ids'] = model_inputs['position_ids'][:, -1:] + 1
        #model_inputs['cache_position'] = model_inputs['position_ids'][-1]
        model_inputs['inputs_embeds'] = torch.cat((model_inputs['inputs_embeds'], next_token_embeds.unsqueeze(1)), dim=1)
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        model_inputs = {
                    #'cache_position': torch.arange(embedding.size(1), device=device), # size of torch.tensor([seq_len])
                    #'past_key_values': None,
                    'input_ids': None,
                    'inputs_embeds': embedding,
                    #'position_ids': torch.arange(embedding.size(1), device=device).unsqueeze(0).expand(embedding.size(0), -1), # size of torch.tensor([batch_size, seq_len])
                    'attention_mask': torch.ones((batch_size, embedding.size(1)), device=device),
                    'logits_to_keep': 1,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=device):
            # prepare model inputs
            #model_inputs = self.prepare_embeds_for_generation(input_ids = output, inputs_embeds = embedding, **model_kwargs)
            #model_inputs.update({'attention_mask': torch.ones((batch_size, embedding.size(1)+index), device=device)})
            # prepare variable output controls (note: some models won't accept all output controls)
            model_inputs.update({"output_attentions": output_attentions} if output_attentions else {})
            model_inputs.update({"output_hidden_states": output_hidden_states} if output_hidden_states else {})
            ...
            this_peer_finished = unfinished_sequences.max() == 0
            index += 1
            model_inputs = self.update_model_inputs(model_inputs, self.model.embed_tokens(next_tokens), device)
            # This is needed to properly delete outputs.logits which may be very large for first iteration
            # Otherwise a reference to outputs is kept which keeps the logits alive in the next iteration
            del outputs
